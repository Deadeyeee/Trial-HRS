...
} from "../clientMessagesCont/Styles";
...
const ClientPaymentInfoCont = () => {
  ...
  const loadingStatus = (value) => {
    if (value == "loading") {
      ...
      return (
        <Grow in={true}>
          <CheckCircleOutline style={{ color: "green", fontSize: "80px" }} />
        </Grow>
      );
    } else if (value == "failed") {
      return (
        <Grow in={true}>
          <HighlightOffSharp style={{ color: "red", fontSize: "80px" }} />
        </Grow>
      );
    }
  };
  ...
  useEffect(() => {
    axios
      .get(apiKey + "auth/verify-token")
      .then((authUser) => {
        console.log(authUser.data);
        axios
          .get(apiKey + "api/getAllReservation")
          .then((getAllReservation) => {
            setReservation([]);
  ...
  useEffect(() => {
    if (reservation.length != 0) {
      new Date(
        Math.max.apply(
          null,
          reservation.map(function (items, index, arr) {
  ...
  const uploadImage = (e) => {
    ...
    if (previewImageError.length == 0 && imageToUpload.length != 0) {
      if (activeReservation.payment.paymentImage != null) {
        ...
        axios
          .patch(
            apiKey + "api/updatePaymentPhoto/" + activeReservation.payment.id,
            formData
          )
          .then((result) => {
            console.log(result.data);
  ...
  const paymentStatusStyle = (value) => {
    if (value.payment.paymentStatus.toUpperCase() == "PENDING") {
        ...
        <div>
          ...
          >
            <p style={{ fontWeight: "normal" }}>
              To confirm your reservation please make your deposit amounting{" "}
              <b>
                {value.payment.paymentType == "Down Payment"
                  ? numberFormat(value.payment.grandTotal / 2)
                  : numberFormat(value.payment.grandTotal)}
              </b>{" "}
      ...
      return (
        <div>
          ...
          >
            <p style={{ fontWeight: "normal" }}>
              {" "}
              Your payment has been varified. Thank you!
            </p>
          </Title>
        </div>
      );
      ...
      return (
        <div>
          ...
          >
            <p style={{ fontWeight: "normal" }}>
              {" "}
              Thank you for choosing RM Luxe Hotel. Unfortunately, your payment
              has been <b>CANCELLED </b> due to unsettled fees. If you have
              already send your proof of payment yet received this message,
              maybe your payment is not yet verified. Kindly disregard this and
              we will send you another email and update your payment status once
      ...
      value.payment.paymentStatus.toUpperCase() == "RECIEPT DECLINED"
    ) {
      return (
        <div>
          <StatusContainer>
          ...
          >
            <p style={{ fontWeight: "normal" }}>
              {" "}
              The proof of payment that you have uploaded may not be verified
              either because of the image quality. Kindly upload a clear and
              valid photo of your receipt for verification. Thank you!
            </p>
          </Title>
        </div>
      );
    ...
    <Container>
      ...
      >
        ...
        >
          ...
          >
            ...
            <h1 style={{ fontWeight: "normal", margin: "0px" }}>
              {loadingMessage}
            </h1>
          </div>
        </Box>
        ...
        <PhotoBox>
          ...
          >
            {activeReservation.payment.paymentImage != null ? (
              <a
                target="_blank"
                href={apiKey + "" + activeReservation.payment.paymentImage}
              >
                <img
                  src={apiKey + "" + activeReservation.payment.paymentImage}
      ...
      {activeReservation.length != 0 ? (
        ...
        >
          ...
          >
            ...
            <a target="_blank" href={previewImage}>
              <img
                src={previewImage}
                width="200px"
                height="auto"
          ...
          {activeReservation.payment.paymentStatus == "cancelled" ||
            ...
            >
              ...
              <Button2
                ...
                fontsize="16px"
                bg={
                  previewImageError.length != 0 || previewImage.length == 0
                    ? "rgb(0,0,0,0.2)"
                    : "#302B20"
                }
                disabled={
      ...
      <MainContainer display="flex" height="auto">
        ...
        <div style={{ width: "100%", overflowY: "auto" }}>
          ...
          >
            <Tr bg="transparent">
              <Th bg="#DFD3B9" color="#2e2e2e" align="center">
                Reservation Number
              </Th>
              <Th bg="#DFD3B9" color="#2e2e2e" align="center">
                Payment Type
